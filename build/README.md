...
`blondi` vytvoří tři **plně samostatné** .exe:
...
| `blondi.exe`    | `main.py` (root)     | PySide6 GUI         | Kompletní Blondi (DB + autonomy+OCR)       |
...
build_all.bat blondi      REM jen blondi.exe
```
...
- **`rh_blondi_paths.py`** — injektuje `_MEIPASS/autonomy` a `_MEIPASS/ocr` na
  `sys.path` (analog `blondi.bootstrap.inject_paths`), přepíše
  `blondi.constants.LOGS_DIR` a `TEMP_ROOT` na exe_dir, změní CWD na
...
- **blondi.exe**: model zůstává v bundlu, cesta přes env proměnnou `OCR_YOLO_MODEL`
...
- **blondi.exe**: ikony se zatím neřeší (resources prázdné), `alembic/` je uvnitř bundlu.
...
4. **Velikost `blondi.exe` (~450 MB)** — důsledek `torch + ultralytics + bosdyn + PySide6`
