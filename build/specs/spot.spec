...
"""PyInstaller spec: spot.exe (PySide6 GUI + autonomy + ocr zabalene uvnitr).

spot.exe je plne samostatny — obsahuje:
  - spot_operator/ balik (entry point je root main.py)
...
Spoustet z root projektu:
    pyinstaller --noconfirm --workpath build/_pyinstaller --distpath dist build/specs/spot.spec
"""
...
hidden = [
    ...
    *collect_submodules("spot_operator"),
...
a = Analysis(
    ...
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[str(HOOK_DIR / "rh_spot_paths.py")],
    excludes=excludes,
    noarchive=False,
    optimize=0,
)
