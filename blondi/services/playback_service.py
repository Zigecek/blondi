...
from blondi.services.map_storage import MapMetadata, load_map_to_temp
from blondi.services.photo_sink import save_photo_to_db
...
class PlaybackService(QObject):
    ...
    def cleanup(self) -> None:
        ...
        if self._map_temp_dir is not None:
            from blondi.services.map_storage import safe_rmtree
    ...
    def _should_retry_outcome(self, result) -> bool:  # noqa: ANN001 — NavigationResult
        ...
        if outcome not in known_outcomes:
            _log.warning(
                ...
                "novější než blondi; outcome nebude retryován.",
                outcome,
            )
            return False
    ...
    def _navigate_with_retry(self, cp: "CheckpointRef", max_retries: int = 2):
        ...
        while attempt <= max_retries:
            ...
            if (
                ...
                try:
                    from blondi.robot.localize_strict import localize_at_start
    ...
    def _localize_with_fallback(self, meta: MapMetadata) -> None:
        ...
        **Proč ne autonomy.localize:** autonomy `SPECIFIC_FIDUCIAL` strategie
        ...
        Fix: vlastní wrapper v `blondi/robot/localize_strict.py` který
    ...
    def _capture_at_checkpoint(self, cp: CheckpointRef) -> CaptureSummary:
        from blondi.robot.dual_side_capture import capture_sources
        from blondi.services.photo_sink import encode_bgr_to_jpeg
